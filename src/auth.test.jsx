...
describe('acesso à conta', () => {
  ...
  it('abre primeiro na página de login e entra com uma conta existente', async () => {
    ...
    expect(screen.getByRole('heading', { name: 'Entre no seu espaço' })).toBeInTheDocument()
    ...
    fireEvent.submit(screen.getByRole('heading', { name: 'Entre no seu espaço' }).closest('.auth-card').querySelector('form'))

    await waitFor(() => expect(screen.getByRole('heading', { name: 'Onde seu negócio está hoje?' })).toBeInTheDocument())
    expect(fetchMock).toHaveBeenCalledWith('/api/auth/login', expect.objectContaining({ method: 'POST' }))
    expect(localStorage.getItem('seu-funcionario-auth-token')).toBe('session-token')
  })
  ...
  it('permite alternar para criação de conta sem esconder o login', async () => {
    ...
    expect(screen.getByRole('heading', { name: 'Crie seu espaço de trabalho' })).toBeInTheDocument()
    ...
    fireEvent.change(screen.getByLabelText(/^Senha/), { target: { value: 'outra-senha-segura' } })
    fireEvent.submit(screen.getByRole('heading', { name: 'Crie seu espaço de trabalho' }).closest('.auth-card').querySelector('form'))

    await waitFor(() => expect(fetchMock).toHaveBeenCalledWith('/api/auth/register', expect.objectContaining({ method: 'POST' })))
    expect(screen.getByRole('heading', { name: 'Onde seu negócio está hoje?' })).toBeInTheDocument()
  })
})
